...
const OBJECTIONS = [
  {
    ...
    back: "17€, c'est le prix d'un déjeuner. Le temps perdu à analyser, douter, tester sans jamais vraiment avancer, lui, il a un prix."
  },
  {
    front: "J'ai déjà tout essayé.",
    back: "T'as peut-être essayé de comprendre encore plus. Pas d'apprendre à lâcher le contrôle sans te perdre. C'est une méthode concrète, pas un texte motivationnel."
  },
  {
    front: "Je n'ai pas le temps.",
    back: "Chez une Cérébrale, « pas le temps » veut souvent dire trop de charge mentale, trop de scénarios. 10 minutes suffisent. Cet e-book ne te prend pas du temps. Il t'en rend."
  },
  {
    front: "Je connais déjà mes schémas.",
    back: "Connaître ses schémas et comprendre pourquoi tu continues à contrôler au lieu de ressentir, c'est pas la même chose. La deuxième, c'est ce qui change vraiment."
  },
]
...
const QUIZ_ITEMS = [
  "Tu es intelligente et lucide, mais émotionnellement souvent tendue, parce que ton esprit ne cesse pas de vérifier si tout est sûr.",
  "Quand quelqu'un te plaît, tu commences à « lire entre les lignes » et tu analyses le ton, le rythme, les mots et les micro-changements de comportement.",
  "Au lieu de ressentir, tu veux d'abord comprendre et anticiper, parce que ça te donne un sentiment de contrôle avant même de t'ouvrir.",
  "Tu as du mal à lâcher le contrôle dans une relation, parce que tu as peur que, si tu lâches, quelque chose te surprenne et te fasse mal.",
  "Quand quelque chose n'est pas clair, la tension monte immédiatement en toi, et ta tête cherche automatiquement des « preuves » de ce que ça signifie.",
  "Quand ça devient plus proche, tu peux te refroidir d'un coup, pas par manque de sentiments, mais pour retrouver du calme à l'intérieur.",
  "Un homme stable et présent peut t'inquiéter, parce que l'absence de drame te paraît parfois suspecte ou « trop simple pour être vrai ».",
  "Il t'arrive de penser : « Peut-être que le problème, c'est moi », parce que si tu vois tout si clairement, pourquoi est-ce encore si difficile d'aimer simplement ?",
]
...
const TOC_ITEMS = [
  "Quand ton mental est devenu ton refuge",
  "Pourquoi la proximité te secoue plus que les autres",
  "Ce que tu protèges vraiment (et ce n'est pas ce que tu crois)",
  "Ton vrai rythme d'attachement (et pourquoi tu le forces)",
  "Les hommes que tu attires (et pourquoi ton mental les choisit)",
  "Le piège de vouloir comprendre avant de partir",
  "Ce que ton corps sait avant ton esprit",
  "Apprendre à rester avec une émotion sans la disséquer",
  "Les hommes qui apaisent ton système",
  "Aimer sans te perdre dans ta tête",
]
...
export default function CerebalePage() {
  ...
  useScrollTracking('cerebrale')

  // Timer
  useEffect(() => {
    let stored; try { stored = localStorage.getItem('oto_cerebrale_end') } catch {}
    let endTime = stored ? parseInt(stored) : Date.now() + 30 * 60 * 1000
    if (!stored) try { localStorage.setItem('oto_cerebrale_end', endTime) } catch {}
    function tick() {
  ...
  function scrollToPaiement() {
    trackEvent('cta_click', { profil: 'cerebrale' })
    paiementRef.current?.scrollIntoView({ behavior: 'smooth' })
  }
  ...
  async function handlePay() {
    ...
    trackEvent('purchase_attempt', { profil: 'cerebrale' })
    setPaying(true)
    ...
    try {
      const res = await fetch(WORKER_URL + '/create-payment-intent', {
        ...
        body: JSON.stringify({ profil: 'cerebrale', email, bump: addBump, bump2: addBump2 })
      })
      ...
      if (confirmResult.paymentIntent?.status === 'succeeded') { trackEvent('purchase_success', { profil: 'cerebrale' }); setShowPopup(true); setPaying(false) }
    ...
    <>
      <div className="card">
        ...
        <div className="narrative fade-section">
          ...
          <p>Tu es intelligente, indépendante, perspicace. De l'extérieur, on dirait que tout est sous contrôle.</p>
          <p>Mais à l'intérieur, tu portes une fatigue particulière : la fatigue d'être celle qui comprend, celle qui anticipe, celle qui tient la barre.</p>
          <p>Tu dis souvent : « Ça va, je gère. » Et pourtant, parfois une pensée te traverse, très discrète mais très vraie : <em>« Est-ce qu'un jour je pourrai aimer… sans devoir tout analyser ? »</em></p>
          <p>Quand quelqu'un se rapproche, <strong>ton mental s'allume.</strong> Tu lis les signaux, tu pèses les mots, tu cherches la cohérence. Tu veux comprendre avant de te laisser ressentir.</p>
          <p>Quand tu commences à t'attacher, ton cerveau veut sécuriser la situation : il prévoit, il teste, il se protège avec des questions.</p>
          <p>Et quand quelque chose te semble flou, tu ne t'effondres pas : tu te retires mentalement. Tu deviens plus rationnelle, plus froide, plus distante, juste pour reprendre le contrôle.</p>
          <p>Et parfois tu regardes d'autres couples en te disant : <em>« Comment ils font pour que ce soit si simple ? »</em></p>
          <p><strong>Le problème, ce n'est pas toi.</strong></p>
          <p>C'est que personne ne t'a jamais appris à te sentir en sécurité sans tout contrôler. À rester ouverte sans t'épuiser à tout décoder. À faire de la place au ressenti, sans perdre ta lucidité.</p>
          <p>Une fois que tu comprends ça, vraiment, quelque chose change. Tu ne deviens pas naïve. Tu ne fermes pas ton intelligence. Tu deviens simplement <em>plus libre.</em></p>
        ...
        <div className="inaction-section fade-section">
          ...
          <div className="inaction-list">
            {[
              "Tu continueras à chercher des preuves partout, même avec quelqu'un de bien.",
              "Tu continueras à « tester » la relation au lieu de la vivre vraiment.",
              "Tu continueras à te refermer dès que c'est flou, pour reprendre le contrôle.",
              "Tu continueras à tout rationaliser, au lieu de dire ce que tu ressens.",
              "Tu continueras à confondre lucidité et méfiance, puis à douter.",
              "Et ton cœur restera protégé, mais aussi privé de repos et de vraie proximité.",
            ].map((item, i) => (
          ...
          <p className="inaction-conclusion">Pas parce que tu es froide. Mais parce que personne ne t'a appris à te sentir en sécurité sans tout contrôler.</p>
            ...
            <div className="book-pages">
              ...
              >
                ...
                {[
                  ...
                  "Ton cerveau va plus vite que ton cœur : tu analyses, tu anticipes, tu cherches du sens partout.",
                  "Tu n'as pas peur d'aimer, tu as peur du flou. Dès que ça devient sérieux, ton radar s'allume.",
                  "Tu as du mal à faire confiance à tes émotions, parce que tu as besoin de clarté.",
                  "Parfois, un homme stable te met mal à l'aise, parce que c'est « trop simple » et tu doutes.",
                  "Tu veux apprendre à ouvrir ton cœur sans perdre ta force, sans armure, mais sans naïveté.",
                ].map((text, i) => (
              ...
              >
                <div className="book-face-front">
                  <div className="book-cover-top">
                    ...
                    <h3 className="book-cover-main-title">Pourquoi tu doutes<br/>tout le temps en amour</h3>
                  </div>
                  ...
                  {[
                    ...
                    "Tu veux des « phrases pour le garder », pas des outils pour apaiser ton système.",
                    "Tu préfères rester dans ta zone de confort émotionnelle, même si elle est solitaire.",
                    "Tu considères la vulnérabilité comme une faiblesse et tu refuses d'y toucher.",
                  ].map((text, i) => (
                    <div key={i} className="book-item">
                      <span className="book-item-icon">✕</span>
                      <span className="book-item-text">{text}</span>
        ...
        <div className="benefits-section fade-section">
          ...
          {[
            "Tu sortiras du mode « cerveau en alerte » et tu apprendras à calmer ton système intérieur, à respirer dans l'incertitude, sans te forcer ni te blinder.",
            "Tu comprendras pourquoi tu contrôles et comment lâcher sans te perdre, rester lucide sans devenir rigide, t'ouvrir sans devenir naïve.",
            "Tu repéreras la cohérence sans t'épuiser à tout analyser, sans décoder chaque emoji, chaque silence.",
            "Tu apprendras à t'attacher sans te fermer, à dire ce que tu ressens sans te justifier, à construire une relation où ton cerveau peut se reposer.",
          ].map((text, i) => (
            ...
            <div className="abri-right">
              <div className="abri-text-wrapper">
                <div className="abri-text-off">
                  <p>Continuer à <strong>analyser, douter, te refermer…</strong></p>
        ...
        <div className="payment-block fade-section" ref={paiementRef} id="paiement">
          ...
          <div className="payment-book-row">
            <div className="payment-book-details">
              <h2>{addBump && addBump2 ? 'Pack 3 e-books' : addBump ? 'Pourquoi tu doutes + Applis de rencontre' : addBump2 ? 'Pourquoi tu doutes + Tu t\'es encore fait ghoster' : 'Pourquoi tu doutes tout le temps en amour'}</h2>
              <p className="offer-sub">{(addBump || addBump2) ? `${1 + (addBump ? 1 : 0) + (addBump2 ? 1 : 0)} e-books · PDF envoyés immédiatement par mail · Mises à jour à vie` : '~170 pages · PDF envoyé immédiatement par mail · Mises à jour à vie'}</p>
            </div>
            <img src="/images/cerebrale.avif" alt="Pourquoi tu doutes tout le temps en amour" className="payment-book-img" width="72" height="113" />
          </div>
            ...
            <div style={{ flexShrink: 0, display: 'flex', justifyContent: 'center', width: '100%', maxWidth: '280px', margin: '0 auto' }}>
              <div className="iphone">
                ...
                <div className="messages" id="abri-msgs">
                  <div className="row" id="abri-m1" style={{ alignItems: 'flex-end', display: 'none' }}><div className="bubble bubble-u">Il a moins écrit ce matin… je sens que quelque chose a changé 😟</div></div>
                  <div className="row" id="abri-t1" style={{ display: 'none' }}><div className="typing"><span /><span /><span /></div></div>
                  <div className="row" id="abri-m2" style={{ alignItems: 'flex-start', display: 'none' }}><div className="sender">Sofi & Oli 💛</div><div className="bubble bubble-a">C'est la première fois que tu ressens ça avec lui ?</div></div>
                  <div className="row" id="abri-m3" style={{ alignItems: 'flex-end', display: 'none' }}><div className="bubble bubble-u">Non… j'analyse tout le temps, je sais plus quoi penser 😞</div></div>
                  <div className="row" id="abri-t2" style={{ display: 'none' }}><div className="typing"><span /><span /><span /></div></div>
                  <div className="row" id="abri-m4" style={{ alignItems: 'flex-start', display: 'none' }}><div className="sender">Sofi & Oli 💛</div><div className="bubble bubble-a">Ton cerveau cherche des preuves. Voilà comment sortir de cette boucle 👇</div></div>
                </div>
      ...
      <div id="sticky-bar-el" className={`sticky-bar${stickyAnimating ? ' animating' : ''}${stickyVisible ? ' visible' : ''}`}>
        <div className="sticky-bar-text">
          Pourquoi tu doutes tout le temps en amour
