...
function Typing() {
  return (
    <div style={{ display: 'flex', gap: 4, padding: '10px 14px', background: 'rgba(102,10,67,0.1)', borderRadius: '18px 18px 18px 4px', width: 'fit-content' }}>
      {[0, 200, 400].map((delay, i) => (
        <span key={i} style={{ width: 6, height: 6, borderRadius: '50%', background: '#660A43', display: 'inline-block', animation: `abri-dot 1.2s ${delay}ms infinite` }} />
      ))}
    </div>
  )
}
...
function IPhoneChat() {
  ...
  useEffect(() => {
    ...
    function run() {
      ...
      timers.push(setTimeout(() => setStep(4), 7000))
      timers.push(setTimeout(() => setStep(5), 8800))
      timers.push(setTimeout(() => setStep(6), 12000))
      timers.push(setTimeout(() => { setStep(0); timers.push(setTimeout(run, 800)) }, 17000))
    }
    run()
  ...
  return (
    <div style={{ width: 260, background: '#FFF1E7', borderRadius: 44, border: '11px solid #1a0812', boxShadow: '0 0 0 1px #3a1020, 0 30px 70px rgba(0,0,0,0.4)', overflow: 'hidden', margin: '0 auto', fontFamily: '-apple-system, BlinkMacSystemFont, "Helvetica Neue", sans-serif' }}>
      ...
      <div ref={msgsRef} style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 7, minHeight: 340, background: '#FFF1E7', overflowY: 'auto' }}>
        {step >= 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
            <div style={{ background: '#660A43', color: '#fff', borderRadius: '18px 18px 4px 18px', padding: '9px 13px', fontSize: 12.5, lineHeight: 1.45, maxWidth: '80%' }}>
              Il me laisse en vu depuis 2 jours… 😞
            </div>
          </div>
        )}
        {step === 2 && <Typing />}
        {step >= 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
            <div style={{ fontSize: 9.5, fontWeight: 700, color: '#660A43', marginBottom: 3, paddingLeft: 2 }}>Sofi & Oli 💛</div>
            <div style={{ background: 'rgba(102,10,67,0.1)', color: '#2a0a1a', borderRadius: '18px 18px 18px 4px', padding: '9px 13px', fontSize: 12.5, lineHeight: 1.45, maxWidth: '80%' }}>
              C'est nouveau chez lui ou il l'a déjà fait avant ?
            </div>
          </div>
        )}
        {step >= 4 && (
        ...
        {step >= 6 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
            <div style={{ fontSize: 9.5, fontWeight: 700, color: '#660A43', marginBottom: 3, paddingLeft: 2 }}>Sofi & Oli 💛</div>
            <div style={{ background: 'rgba(102,10,67,0.1)', color: '#2a0a1a', borderRadius: '18px 18px 18px 4px', padding: '9px 13px', fontSize: 12.5, lineHeight: 1.45, maxWidth: '80%' }}>
              Les hommes se retirent rarement par indifférence. Voilà quoi faire 👇
            </div>
          </div>
        )}
      </div>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto', width: '100%' }}>
          <div className="hp-hero-cols" style={{ display: 'flex', alignItems: 'center', gap: 60 }}>
            <div style={{ flex: 1 }}>
              ...
              <h1 style={{ fontFamily: 'var(--font-playfair,serif)', color: '#FFF1E7', fontSize: 'clamp(32px,5vw,62px)', fontWeight: 700, lineHeight: 1.15, marginBottom: 24 }}>
